...
; flights will be a map:  keys : (id,from,to,carrier) values: flightatom (map id,from,to,carrier,pricing)   (only keep pricing maybe?)
...
(defn make-flight [id from to carrier pricing]
  (atom  {:id  id, :from from, :to to, :carrier carrier, :pricing pricing}))

(def flights '())

;we assume that initialize-flights won't be called in parallel, this wouldn't make sense anyway
(defn initialize-flights [initial-flights]
  ;"Set `flights` atom to the `initial-flights`."  ;calva gives docstring error
  (set! flights initial-flights))
